# COBOL Documentation Workflow

This document describes the step-by-step process for using AI to document COBOL programs effectively, following industry best practices for legacy COBOL codebase documentation.

## Overview

The COBOL Documentation Workflow transforms undocumented or poorly documented COBOL programs into well-documented, maintainable codebases through a fully automated process by:
1. Analyzing and chunking COBOL program structure
...
**CRITICAL**: Throughout this workflow, `${COBOL_DOCS_DIR}` refers to the `cobol-documentation/{REPO-NAME}/[PROGRAM-NAME]/` directory structure at the project root. All program-specific documentation artifacts MUST be placed within the program's subdirectory under its repository folder, never at the project root level.

**Directory Structure**:
- `cobol-documentation/` - Root directory for all COBOL documentation
- `cobol-documentation/{REPO-NAME}/` - Repository-specific documentation folder
- `cobol-documentation/{REPO-NAME}/[PROGRAM-NAME]/` - Program-specific documentation folder
- `cobol-documentation/{REPO-NAME}/standards/` - Repository-specific standards and guidelines (optional)
- `cobol-documentation/{REPO-NAME}/training/` - Repository-specific training materials (optional)
- `cobol-documentation/standards/` - Cross-repository shared standards (optional)

**Example paths**:
- Data dictionary: `cobol-documentation/cobol-programming-course/CBL0011/CBL0011_DATA_DICTIONARY.md`
- Call graph: `cobol-documentation/cobol-programming-course/CBL0011/CBL0011_CALL_GRAPH.md`
- Comprehensive doc: `cobol-documentation/cobol-programming-course/CBL0011/CBL0011_COMPREHENSIVE_DOC.md`
- Paragraph docs: `cobol-documentation/cobol-programming-course/CBL0011/paragraphs/PROCESS-RECORDS.md`

## Phase 1: Program Analysis and Chunking

### 1.1 COBOL Program Structure Analysis
- **Objective**: Understand the COBOL program's organization, divisions, sections, and logical boundaries
- **Actions**:
  - Identify IDENTIFICATION DIVISION metadata (program name, author, date written)
  - Analyze ENVIRONMENT DIVISION (file assignments, special names)
  - Catalog DATA DIVISION structure (FILE SECTION, WORKING-STORAGE SECTION, LINKAGE SECTION)
  - Map PROCEDURE DIVISION organization (sections, paragraphs, PERFORM hierarchy)
  - Assess program size and complexity (lines of code, number of paragraphs/sections)
  - Identify external dependencies (COPY books, called programs, file I/O)

### 1.2 Chunking Strategy Implementation
- **Objective**: Break large programs into logical, manageable sections for documentation
- **Chunking Approach**:
  - **By Division**: Document each COBOL division separately
  - **By Section**: For large PROCEDURE DIVISIONs, document each section independently
  - **By Logical Function**: Group related paragraphs by business function
  - **By Data Group**: Group related data items in WORKING-STORAGE by purpose
  - **By File Processing**: Document file handling logic as discrete units
  
**CRITICAL**: Large monolithic COBOL programs (>1000 lines) must be chunked to avoid overwhelming the AI context window and to enable focused documentation.
...
- **Actions**:
  ...
  - **PERFORM Hierarchy**: Map all PERFORM statements to create call graph
  - **File Operations**: Identify all READ, WRITE, REWRITE, DELETE operations
  - **Conditional Logic**: Catalog all IF, EVALUATE, and conditional PERFORM statements
...
**Recommended Tools**:
- COBOL compilers with cross-reference listings (-Xref flags)
- Static analysis tools (IBM Debug Tool, Micro Focus Animator)
- Custom scripts to parse COBOL source and generate usage reports
...
- **Actions**:
  - **Create master index document**: `${COBOL_DOCS_DIR}/[PROGRAM-NAME]_INDEX.md`
  - List all divisions, sections, paragraphs with line numbers
...
- Dependencies: [COPY books, called programs]

## Divisions and Sections
1. DATA DIVISION
   - WORKING-STORAGE SECTION (Lines XX-YY) → [Link to documentation]
   - FILE SECTION (Lines AA-BB) → [Link to documentation]
2. PROCEDURE DIVISION
   - Section A (Lines CC-DD) → [Link to documentation]
   - Section B (Lines EE-FF) → [Link to documentation]

## Static Analysis References
- Variable Cross-Reference: [Link to report]
- PERFORM Hierarchy: [Link to call graph]
- File Operations Summary: [Link to I/O report]
```

## Phase 2: Iterative Documentation Generation

### 2.1 Generate Data Dictionary (WORKING-STORAGE)
**First Documentation Priority**: Document all data structures before procedural code

**Process**:
1. **Extract WORKING-STORAGE SECTION**: Copy the complete section for AI analysis
...
3. **Generate data dictionary entries** for each data item:
   - Variable name and level number
   - Data type and size (PIC clause)
   - Purpose and usage description
   - Initial value (VALUE clause)
   - Where read/modified (from cross-reference)
   - Special handling notes (REDEFINES, OCCURS, COMP fields)

4. **Document related data groups**: Group related fields (e.g., customer records, date fields, counters)
5. **Document error handling variables**: Identify FILE STATUS variables, error flags, return codes
6. **Document buffer sizes and limits**: Note maximum sizes, overflow risks, validation requirements
7. **Create data dictionary document**: `${COBOL_DOCS_DIR}/[PROGRAM-NAME]_DATA_DICTIONARY.md`

**AI Prompt Template**:
```
Given this COBOL WORKING-STORAGE section and the variable cross-reference report,
...
1. **FILE STATUS Analysis**:
   - Identify all FILE STATUS clauses (or note their absence as risk)
   - Document expected status codes and handling procedures
   - Note file operations without error checking
   - Document recovery procedures for file errors

2. **Runtime Error Scenarios**:
   - File not found or access denied
   - Invalid data format or type mismatch
   - Buffer overflow conditions
   - Divide by zero or arithmetic errors
   - String overflow in STRING operations
   - UNSTRING pointer errors

3. **Resource Limit Documentation**:
   - Maximum file sizes supported
   - Buffer size limitations and overflow risks
   - Array bounds and OCCURS limits
...
5. **Risk Assessment**:
   - High Risk: No FILE STATUS handling, buffer overflows possible
   - Medium Risk: Limited validation, missing error recovery
   - Low Risk: Comprehensive error handling implemented

**Create error handling document**: `${COBOL_DOCS_DIR}/[PROGRAM-NAME]_ERROR_HANDLING.md`

**AI Prompt Template**:
```
Analyze this COBOL program for error handling and risks:

1. Identify all FILE STATUS clauses and error checking
...
[Insert FILE SECTION]
[Insert file I/O paragraphs]
```
   
3. **Generate paragraph documentation**:
...
4. **Create individual paragraph document**: `${COBOL_DOCS_DIR}/paragraphs/[PARAGRAPH-NAME].md`

**AI Prompt Template**:
```
Document this COBOL paragraph with the following context:
- Data dictionary for variables used: [link]
- PERFORM hierarchy: [link to callers/callees]
- Static analysis: [relevant analysis]

Explain:
1. What this paragraph does (business purpose)
...
[Insert paragraph source code]
```

**Chunking Guidance**:
- Document 1-3 related paragraphs per AI interaction
- Keep context window manageable (<4000 tokens per request)
- Document called paragraphs before calling paragraphs when possible

### 2.3 Create Call Graphs (PERFORM Relationships)
**Third Documentation Priority**: Map program control flow

**Process**:
1. **Generate PERFORM hierarchy tree** from static analysis
2. **Feed to AI with paragraph documentation** already created
3. **Create visual call graph** showing:
   - Entry points (main program flow)
   - PERFORM chains and nesting
   - Conditional PERFORM paths
   - Loop structures (PERFORM UNTIL, PERFORM VARYING)
   - Exit points and termination conditions

4. **Create call graph document**: `${COBOL_DOCS_DIR}/[PROGRAM-NAME]_CALL_GRAPH.md`
...
1. MAIN-PROCESSING
   - PERFORMS INITIALIZATION
     - PERFORMS OPEN-FILES
     - PERFORMS LOAD-PARAMETERS
   - PERFORMS PROCESS-RECORDS (UNTIL end-of-file)
     - PERFORMS READ-INPUT-FILE
     - PERFORMS VALIDATE-RECORD
       - PERFORMS CHECK-REQUIRED-FIELDS
       - PERFORMS VALIDATE-AMOUNTS
     - PERFORMS WRITE-OUTPUT-FILE
   - PERFORMS CLEANUP
     - PERFORMS CLOSE-FILES
     - PERFORMS WRITE-SUMMARY

## Loop Structures
- PROCESS-RECORDS: PERFORM UNTIL WS-EOF-FLAG = 'Y'
- ARRAY-PROCESSING: PERFORM VARYING WS-INDEX FROM 1 BY 1 UNTIL WS-INDEX > 100
```
...
2. **Feed to AI with full context**:
   ...
   - All paragraphs that modify each variable
...
4. **Create mutation analysis document**: `${COBOL_DOCS_DIR}/[PROGRAM-NAME]_VARIABLE_MUTATIONS.md`
...
- All paragraphs that modify it (in likely execution order based on call graph)
...
[Insert relevant paragraph documentation]
...
1. **Data Dictionary Validation**:
   ...
   - Verify data type interpretations against PIC clauses and VALUE statements
   - Validate usage patterns match cross-reference reports
   - Identify and document all variable relationships

2. **Paragraph Documentation Validation**:
   ...
   - Cross-check COBOL syntax interpretation with language specifications

3. **Call Graph Validation**:
   - Verify PERFORM relationships by parsing all PERFORM statements
   - Confirm conditional execution paths by analyzing IF/EVALUATE statements
   - Validate loop structures by checking PERFORM UNTIL/VARYING syntax
   - Identify all control flow paths including GO TO statements

4. **Variable Mutation Analysis Validation**:
   - Verify state transitions by tracing all MOVE and COMPUTE statements
...
**Validation Report**: Automatically generate `${COBOL_DOCS_DIR}/[PROGRAM-NAME]_VALIDATION_REPORT.md` containing:
...
3. **Resolve ambiguities**: Apply COBOL best practices and common patterns to resolve unclear logic
...
- **Technical Corrections**: Fix COBOL syntax misinterpretations using language specifications
...
- Standardized terminology for COBOL constructs (DIVISION, SECTION, PARAGRAPH)
- Appropriate detail level based on program complexity assessment
- Common COBOL pattern documentation using established templates
- Cross-reference formatting and linking conventions

**Auto-generate standards document**: `cobol-documentation/DOCUMENTATION_STANDARDS.md` (at root level, shared across all programs) documenting:
- Format conventions used in this documentation
- Terminology definitions and usage
- Documentation depth rationale
- COBOL pattern templates applied
...
2. **Generate comprehensive program documentation** that includes:
   ...
   - **Dependencies**: External programs, COPY books, files, databases
...
3. **Create master program document**: `${COBOL_DOCS_DIR}/[PROGRAM-NAME]_COMPREHENSIVE_DOC.md`

**AI Prompt for Synthesis**:
```
Using all the component documentation we've created (data dictionary, paragraph docs,
...
[Link to paragraph documentation folder]
[Link to call graphs]
...
2. **Paragraph Index**: Every paragraph → purpose, callers, callees, documentation link
3. **File Operations Index**: Every file → READ/WRITE locations, paragraph documentation
...
**Create cross-reference document**: `${COBOL_DOCS_DIR}/[PROGRAM-NAME]_CROSS_REFERENCE.md`
...
2. **Calling Program Contract**:
   - Entry point parameters (LINKAGE SECTION)
...
4. **Copybook Dependencies**:
   - All COPY statements
   - Copybook purposes and contents
   - Shared data structure definitions
   - Version compatibility requirements

5. **System Requirements**:
   - COBOL compiler version (e.g., GnuCOBOL 3.x)
   - Operating system requirements
   - Required compile flags
   - Runtime environment settings
   - JCL or script requirements
...
**Create integration document**: `${COBOL_DOCS_DIR}/[PROGRAM-NAME]_INTEGRATION_GUIDE.md`

**AI Prompt Template**:
```
Document all integration points for this COBOL program:

1. List all CALL statements with parameter details
2. Document LINKAGE SECTION contract (entry point interface)
3. List all COPY statements and their purposes
4. Document file dependencies (input/output)
5. Identify compiler and system requirements
6. Document deployment and configuration needs

[Insert program source]
[Insert LINKAGE SECTION]
[Insert all CALL statements]
[Insert all COPY statements]
```
...
2. **Implicit Business Rules** (inferred from code):
   - Comment syntax (e.g., ";" starts LISP comments)
...
**Create business rules document**: `${COBOL_DOCS_DIR}/[PROGRAM-NAME]_BUSINESS_RULES.md`

**AI Prompt Template**:
```
Extract and document all business rules from this COBOL program:
...
[Insert business logic paragraphs]
```
...
**Add to diagrams document**: `${COBOL_DOCS_DIR}/[PROGRAM-NAME]_MERMAID_DIAGRAMS.md`

**AI Prompt Template**:
```
Create data flow visualizations for this COBOL program:
...
**Objective**: Create Mermaid-based visual representations for every COBOL program to aid modernization, knowledge transfer, and understanding of legacy code structure
...
1. **Program Flow Diagram (flowchart)**:
   ...
   - Decision points (IF/EVALUATE statements)
   - Loop structures (PERFORM UNTIL/VARYING)
...
2. **PERFORM Hierarchy (graph)**:
   - All paragraph call relationships
   - Nested PERFORM structures
   - Execution order and dependencies
   - Loop paragraphs vs one-time execution
...
3. **Data Flow Diagram (flowchart)**:
   ...
   - Data transformations through WORKING-STORAGE
   ...
   flowchart LR
       ...
       Read --> WS[WORKING-STORAGE<br/>Data Transformation]
       WS --> Calc[Business Logic<br/>Calculations]
       Calc --> Format[Format Output]
       Format --> Write[Write Records]
       Write --> Output[(OUTPUT-FILE)]
   ```

4. **Copybook Dependency Graph (graph)**:
   - All COPY statements and their relationships
   - Shared data structures across programs
   - Nested copybook inclusions
   - Dependency hierarchy

   ```mermaid
   graph TB
       Program[MAIN-PROGRAM] --> Copy1[COPY CUSTOMER-REC]
       Program --> Copy2[COPY DATE-ROUTINES]
       Program --> Copy3[COPY ERROR-CODES]
       Copy2 --> Copy4[COPY COMMON-UTILS]
       Copy3 --> Copy4
   ```
...
```
Using the call graph, data dictionary, and paragraph documentation, generate Mermaid diagrams for this COBOL program:

1. Program Flow (flowchart): Main processing logic with decision points
2. PERFORM Hierarchy (graph): All paragraph call relationships
3. Data Flow (flowchart): Input → Processing → Output flow
4. Copybook Dependencies (graph): All COPY statements and relationships
...
[Insert COPY statements list]
```

**Output Structure**:
Create **mandatory** diagrams document: `${COBOL_DOCS_DIR}/[PROGRAM-NAME]_MERMAID_DIAGRAMS.md`
...
- Error handling redirects to logging paragraph
...
**Actions**:
1. Update `${COBOL_DOCS_DIR}/[PROGRAM-NAME]_INDEX.md` with links to:
...
2. Add navigation paths for different user types:
   - **New developers**: Start here path (executive summary → architecture → Mermaid diagrams → key paragraphs)
   - **Maintenance developers**: Quick reference path (cross-reference → specific paragraph → testing guide)
...
1. **When to Update Documentation**:
   - After any paragraph/section modification
   - When adding new variables to WORKING-STORAGE
...
**Create maintenance guide**: `cobol-documentation/MAINTENANCE_GUIDE.md` (at root level, shared across all programs)

**AI Prompt Template for Maintenance**:
```
The following paragraph was modified. Update its documentation accordingly.
...
1. **Coverage Metrics**:
   - % of paragraphs documented
   - % of WORKING-STORAGE variables documented
   - % of PERFORM relationships mapped
...
│   └── Document paragraphs individually
└── Large Programs (>2000 lines)
    ├── Chunk by division first
    ├── Sub-chunk PROCEDURE DIVISION by section
    ├── Document related paragraph groups (3-5 paragraphs)
...
├── Compiler with Cross-Reference Available
│   └── Use compiler -Xref output as primary source
├── Static Analysis Tool Available (IBM Debug, Micro Focus)
│   └── Generate comprehensive reports before AI documentation
├── No Tools Available
│   ├── Parse COBOL manually with custom scripts
...
│   ├── Standard documentation (data dictionary + paragraph docs + call graph)
│   ├── Single expert review
│   └── Basic maintenance guide
└── Simple Utility Programs
    ├── Minimal documentation (overview + key paragraph docs)
...
- [ ] Data dictionary completed for all WORKING-STORAGE (with buffer limits documented)
- [ ] Each paragraph/section documented individually (with error handling and performance notes)
...
- Reduced time to understand and modify COBOL programs
...
**Issue**: AI misinterprets COBOL syntax or semantics
**Solutions**:
- Automatically detect COBOL dialect from source code indicators
- Include COBOL language reference in AI context automatically
- Use comprehensive COBOL specifications for validation
...
**Solutions**:
- Verify COBOL compilation settings
- Check for compiler directives affecting analysis
...
### For Using AI to Document COBOL
...
3. **Iterative Approach**:
   - Start with data dictionary (WORKING-STORAGE)
   - Document each paragraph/section individually
   - Create call graphs showing PERFORM relationships
...
After completing COBOL program documentation:

1. **Apply to More Programs**: Use lessons learned to document additional COBOL programs
...
5. **Cross-Reference Systems**: Link related programs and shared COPY books
